...
func (s *session) loadPlayerState(ctx context.Context, guid uint64) (playerState, error) {
	...
	var race, class, gender, skin, face, hairStyle, hairColor, facialStyle, level, playerFlags, mapID, extraFlags, atLogin, zone, deathExpireTime, cinematic int64
...
func (s *session) calculatePlayerStats(ctx context.Context, state *playerState) error {
	...
	for _, item := range equippedItems {
		...
		// Stats
		for k := 0; k < 10; k++ {
			s.applyPlayerItemStat(state, uint32(st[k]), sv[k])
		}
...
func (s *Server) buildPlayerUpdateForRecipient(state playerState, targetSelf, partyMember bool, runtime *session, recipient *playerState) (*protocol.Packet, error) {
	...
	if state.TransformDisplayID != 0 {
		values[unitFieldDisplayID] = state.TransformDisplayID
	}
	...
	if state.Level >= 10 {
		...
		if state.Class == 6 && state.Map == 609 && state.Level >= 56 {
			totalPoints = uint32(state.Level-55) + state.QuestBonus
	...
	if flags&0x0004 != 0 {
		block.WritePackedGUID(runtime.attackTarget)
	}
...
func IsPlayerFieldVisibleToRecipient(index int, targetSelf, partyMember bool) bool {
	return playerFieldVisibleToRecipient(index, targetSelf, partyMember)
}
...
func PlayerCreateUpdateFlags(targetSelf, hasVictim bool) uint16 {
	flags := uint16(0x0060)
	if targetSelf {
		flags |= 0x0001
	}
...
func (s *Server) buildPlayerValuesUpdateForRecipient(guid uint64, fields map[int]uint32, targetSelf, partyMember bool, source, recipient *playerState) (*protocol.Packet, error) {
	if !targetSelf {
		...
		for index, value := range fields {
			if playerFieldVisibleToRecipient(index, false, partyMember) {
				public[index] = value
			}
		}
		if len(public) == 0 {
...
func (s *Server) broadcastPlayerValuesUpdateFromSession(source *session, fields map[int]uint32) {
	if s == nil || source == nil || source.player == nil || s.Config.VisibilityDistanceContinents <= 0 {
		return
	}
	state := *source.player
	distance := float64(s.Config.VisibilityDistanceContinents)
	...
	for target := range s.sessions {
		...
		target.visiblePlayersMu.Lock()
		_, visible := target.visiblePlayers[state.GUID]
		target.visiblePlayersMu.Unlock()
		if !visible {
			continue
		}
