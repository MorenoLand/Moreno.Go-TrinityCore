...
Item 5 active milestone: the earlier state bundle restores ranked default-skill tiers, reputation flags, death expiry, taxi routes/masks, group leadership, rested state, pet gates, nested-bag GUIDs, enchant durations, mail attachment fields, and periodic quest cooldowns. This follow-up adds DBC-backed item-limit-category decoding and equipped-slot/use/uniqueness checks, restores recursive prior spell ranks, and loads saved quest item/player counters plus exploration/timer/money/reputation completion requirements. Logincheck self-checks exercise the ItemLimitCategory DBC record. Full bag-family/count-limit/mail transaction parity, all-rank stackability edge rules, live quest item/PvP counter transitions, and the real-character DB plus client packet replay diff remain open.
