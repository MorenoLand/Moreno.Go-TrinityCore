...
Item-5 stat persistence: Go now derives `CanParry`/`PLAYER_PARRY_PERCENTAGE` from source effect 22, class caps/diminishing values, DBC combat ratings, defense skill, and aura 47, recalculating on relevant effect/aura changes. It maps `PlayerSave.Stats.MinLevel` and `PlayerSave.Stats.SaveOnlyOnLogout`, preserves the disabled/logout-only defaults, and writes the 31 source-ordered values to `character_stats` in the logout transaction. A copied real-character replay with minimum level 1 inserted exactly one row; a fresh default-config replay left the table unchanged. Logincheck rejects unowned columns and verifies the default config. Full stat-producer and character-state parity remain open.
...
Item 5 remains active: `character_stats` has no source-equivalent writer/config gates (Go lacks a stored parry-percent field); non-inventory relations still use aggregate/column digests and the replay does not assert every required source write; broader auth/account data state remains incomplete; group reward packet/state replay is incomplete; and compatible-client acceptance has not been run.
