...
Item-5 replay snapshot coverage now includes account data, account instance times/tutorials, arena-team membership, guild withdrawal counters, sent/received mail, received-mail attachment `item_instance` rows, player auction/bid rows and attached items, and calendar event/invite rows. A selected local real-character replay passed with 57 server packets and no table/column changes after adding those queries. The snapshot still needs a complete source-owned table inventory, expected-state assertions for every login mutation, and compatible-client acceptance; remaining GUID-linked tables require explicit source-owner review before inclusion.
