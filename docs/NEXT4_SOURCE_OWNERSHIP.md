...
Creature pet casting now follows the source `HandlePetCastSpellOpcode` ownership boundary: active pet motion state retains all learned `pet_spell` IDs, charmed or owned motion is required, passive/unknown spells are rejected, cooldowns are checked, `SMSG_PET_CAST_FAILED` is returned for rejected requests, and direct damage effects execute with the creature GUID as caster and update combat logs, player victims, creature health, threat, and loot state. Non-damage pet effects and complete `Spell::CheckPetCast` target/mechanic coverage remain open for later item-4 spell parity.
