...
2026-09-24 item-4 re-audit: `CharacterHandler::HandlePlayerLogin` adds the player to the map only after pre-map packets and the self-create/map-entry transition; Go had set `playerLoaded` before sending that create update, allowing background creature AI and world broadcasts to observe the character too early. Atomic `worldReady` now gates creature targeting, combat/time/regen/death/drowning ticks, visibility/movement, group/social/chat/channel, guild/petition, transport/vehicle/totem, battleground, and world-state fan-outs; it opens only after the self-create update and closes on far transfer/logout. The contested-PvP timer starts after active-pet restoration, matching source order. Logincheck rejects attack/damage/spell events before self-create; its in-process two-session fanout fixture proves attack, group-list, and guild-signed-on broadcasts are withheld from an unready peer and delivered once after readiness. All 27 isolated character login/logout replays and repository, race, vet, project, root, and Linux builds pass. Full multi-session login/petition sequences and compatible-client acceptance remain unverified, so item 4 stays active.

World-ready fanout replay details: after each real-character login, `validateWorldReadyFanout` inserts a pre-create peer and synthetic group/guild sender, then invokes the actual nearby attack, `SMSG_GROUP_LIST`, and guild-signed-on broadcast functions. The trace recorder is isolated from the login stream; no packet reaches the unready peer, while each broadcast reaches it exactly once after `worldReady` flips. This is a fanout fixture rather than a full two-client login or petition workflow.
