...
Item 5 remains active: `character_stats` has no source-equivalent writer/config gates (Go lacks a stored parry-percent field), the replay uses aggregate digests rather than keyed row semantics and accepts arbitrary inventory deletions, account DB and other-character online effects are not snapshotted, group reward packet/state replay is incomplete, and no compatible client acceptance has been run.
