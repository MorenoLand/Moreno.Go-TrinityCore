...
Latest item-5 follow-up: quest progress now uses TrinityCore's add/remove delta rules across bank and buyback, vendor costs/sales, ammo, item use/destroy/refund, wrapping, sockets, mail, auctioning, trade, full-stack guild-bank swaps, quest rewards, zone-limited destruction, and offline timed-item expiry. Quest acceptance alone counts bank contents; removal uses carried inventory only when the persisted objective count is already full. Logincheck also identifies the self-player create by the requested GUID, parses transport and spline movement tails, distinguishes the source login-effect spell from earlier zone-triggered spells, and stops the initial movement-order check at the source quest-status boundary. Its self-check covers these parser/order rules and quest-count arithmetic. An isolated replay batch passed all 27 active character rows; the core replay disables Eluna, performs disconnect cleanup, guards character-row presence, and reports changed tables/columns without values. The database snapshot now includes all character-owned tables present in the local schema plus linked pet, inventory-item, refund, and soulbound-trade rows; source-vs-Go expected-state coverage is still incomplete. Item 5 remains open for compatible-client acceptance, source-defined guild-bank split amounts, and that full persistence diff; in-process replay alone is not client proof.
