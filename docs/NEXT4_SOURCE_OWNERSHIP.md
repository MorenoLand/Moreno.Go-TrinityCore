...
Item-5 stat persistence: Go derives `CanParry`/`PLAYER_PARRY_PERCENTAGE` from source effect 22, class caps/diminishing values, DBC combat ratings, defense skill, and aura 47, recalculating on relevant effect/aura changes. `PlayerSave.Stats.MinLevel` and `SaveOnlyOnLogout` retain disabled/logout-only defaults; qualified logout writes the 31 source-ordered values to `character_stats`. The equipped stat loader now reads character inventory from the character store and item templates from the world store, then adds fixed spell-power stat effects in permanent/temporary enchant slots using `SpellItemEnchantment.dbc`, durability, level, and required-skill gates. Enabled/default real-character replays pass, and logincheck compares the persisted base spell power with the equipped template/enchant calculation. Conditioned enchants, gem/socket effects, and random-property/suffix scaling remain open, as does full character-state parity.
