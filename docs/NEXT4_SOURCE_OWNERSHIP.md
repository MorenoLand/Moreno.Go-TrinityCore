...
Latest item-5 follow-up: persisted item-count refresh now runs on the major acquisition/removal paths for loot, vendor store/sale, mail, trade, guild-bank moves, auction listing, item use/destroy/refund, wrapping, sockets, quest-start items, and quest reward consumption. Item 5 remains open for any unreviewed direct inventory mutation, quiver/group-raid edge cases, and the missing real-character DB/client trace diff.
