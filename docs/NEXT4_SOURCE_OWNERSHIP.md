...
Item 4 closeout verification: the pinned `CharacterHandler.cpp`, `Player::SendInitialPacketsBeforeAddToMap`, and `Player::SendInitialPacketsAfterAddToMap` flow was compared with the Go login sequence and existing `tools/logincheck` ordered packet/payload fixtures. The final recipient-specific group fields follow `Unit::BuildValuesUpdate` and `FactionTemplateEntry::IsFriendlyTo`; logincheck validates both hostile cross-faction masking and unchanged friendly/disabled/self cases. `go run ./tools/logincheck --self-check`, `go test ./...`, `go test -race ./...`, `go vet ./...`, `scripts\build.ps1 moreno`, `go build -trimpath -o bin\MorenoCore.exe .`, and `git diff --check` pass.

Item 5 active milestone: the current source pass restores ranked default-skill tier/value calculation, total-standing hostile reputation flags, rank-bounded death-expiry, source-order taxi-mask initialization, active taxi-route persistence, group-leader create flags, offline rest rates/logout resting state, temporary-pet unsummon gates, nested-bag GUID resolution, timed enchant updates for all inventory locations, complete mail attachment fields, and daily/weekly/monthly/seasonal quest cooldown persistence. Logincheck exercises the DBC-backed skill tier and state fixtures; equipment eligibility/mail recovery, persisted spell-chain reconstruction, quest objective counters, and the remaining real-character DB/packet diffs stay open.
