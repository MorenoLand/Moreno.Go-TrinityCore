...
Pet persistence now runs before the Go pet is unsummoned, saving the modeled character_pet fields and pet aura/cooldown rows from the active motion state; 27/27 isolated login/logout replays pass. Pet mana, experience, happiness, and full category-cooldown runtime state are not represented by the current Go pet model and remain open. Corpse insertion now stores the source-equivalent phase mask by combining active phase-aura misc values, defaulting to PHASEMASK_NORMAL, and using the GM phase mask when enabled; global three-day corpse cleanup remains a later lifecycle item. Item 5 remains active for the legacy SQLite caster-GUID load case, full saved-state comparison, and compatible-client acceptance.
