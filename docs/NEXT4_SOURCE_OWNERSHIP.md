...
Creature-target charm now marks the live creature controlled/possessed, changes its faction to the charmer, clears combat, sends `SMSG_CLIENT_CONTROL_UPDATE` and a source-shaped `SMSG_PET_SPELLS` charm bar, and restores the original creature state when the aura expires or is dispelled, matching `Unit::SetCharmedBy`, `CharmSpellInitialize`, and `RemoveCharmedBy` (creature charm-control correction).
