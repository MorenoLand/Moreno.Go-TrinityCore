...
Item 5 active milestone: the earlier state bundle restores ranked default-skill tiers, reputation flags, death expiry, taxi routes/masks, group leadership, rested state, pet gates, nested-bag GUIDs, enchant durations, mail attachment fields, and periodic quest cooldowns. This follow-up adds DBC-backed item-limit-category decoding; item eligibility, count/category caps, bag family/capacity, socket-gem uniqueness, and atomic mail recovery; recursive prior spell ranks with DBC stackability and talent-rank selection; and saved quest item/player counters with exploration/timer/money/reputation completion checks plus zone-gated PvP credit. Logincheck exercises the ItemLimitCategory DBC and rank-stackability records. Remaining item-state gaps include full live item-count transitions, group/raid PvP credit gates, scaling-item/quiver edge cases, and a real-character DB plus client packet replay diff; the local bin contains no recorded trace for that replay.
