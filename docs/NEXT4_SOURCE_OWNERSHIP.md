...
2026-09-24 item-4 re-audit: `CharacterHandler::HandlePlayerLogin` adds the player to the map only after pre-map packets and the self-create/map-entry transition; Go had set `playerLoaded` before sending that create update, allowing background creature AI and world broadcasts to observe the character too early. An atomic `worldReady` gate now begins immediately after the self-create update, is cleared on far teleport/logout, and gates creature targeting, time/combat/regeneration ticks, and selected movement/player-create fan-outs. The persisted contested-PvP timer now begins after active-pet restoration, matching the source post-map order. Logincheck rejects attack/spell events before self-create; all 27 isolated character login/logout replays and the required compile/build/self-check gates pass. Item 4 remains ACTIVE pending the remaining packet fan-out audit and client acceptance.
