...
Item-5 stat persistence: Go derives `CanParry`/`PLAYER_PARRY_PERCENTAGE` from source effect 22, class caps/diminishing values, DBC combat ratings, defense skill, and aura 47, recalculating on relevant effect/aura changes. `PlayerSave.Stats.MinLevel` and `SaveOnlyOnLogout` retain disabled/logout-only defaults; qualified logout writes the 31 source-ordered values to `character_stats`. The equipped stat loader reads character inventory from the character store and item templates from the world store, then adds fixed spell-power effects from permanent/temporary slots plus positive random-property and negative random-suffix DBC entries. Suffix factor calculation shares the source-equivalent resolver used for item display. Enabled/default replays and a real-character positive random-property replay pass; logincheck checks the saved base spell power against equipped templates and DBC effects. The local character fixture has no equipped negative random-suffix item, so that path has DBC/self-check evidence but no real-character replay. Conditioned enchants, gem/socket effects, non-spell-power random-affix stats, and full character-state parity remain open.
