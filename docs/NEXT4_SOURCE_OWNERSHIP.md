...
The final item-4 source re-audit compared `CharacterHandler::HandlePlayerLogin`, `Player::SendInitialPacketsAfterAddToMap`, `Map::AddPlayerToMap`, and `Spell::SendSpellGo` with the Go login stream. It found that login spell 836 was built from the character's class power and wall-clock milliseconds, while TrinityCore uses the spell's DBC `PowerType`, application-monotonic milliseconds, and `CAST_FLAG_NO_GCD` when `StartRecoveryTime` is zero; the trace checker also accepted a missing, pre-map, or mis-targeted login effect. The local `Spell.dbc` record 836 has `PowerType=0` and `StartRecoveryTime=0`. Go now sends the DBC-selected power to the player only, uses the monotonic game-time clock, applies the no-GCD flag, and keeps first-login triggered self-casts on self-unit targets with the source cast ID and observer power-field rules. `logincheck` requires exactly one post-map spell 836 from and to the logging-in player and checks its source flags; adversarial fixtures reject omission, pre-map placement, wrong target, and missing no-GCD. The focused self-check, isolated real-character replay, repository gates, project build, and root build pass. Item 4 is closed; item 5 remains first unfinished for persisted character-state coverage.
...
The NPCBot XP modifier now counts owner-controlled runtime creature motions whose entries exist in the loaded NPCBot template data, rather than every persisted ownership row; focused checks cover foreign-owner motions, temporary/template entries, pets, duplicate GUIDs, and the reduction floor. This matches the `BotMgr::_bots` counting boundary for represented runtime motions, but the Go server currently lacks NPCBot runtime creation/registration, so real roster parity remains open. Character saves now perform the source delete/reinsert for `character_battleground_data` and persist non-empty `account_instance_times` maps in the same transaction. The account time mutation on dungeon entry and battleground return-point updates are still missing.

Item 5 remains active: `character_stats` has no source-equivalent writer/config gates (Go lacks a stored parry-percent field); non-inventory relations still use aggregate/column digests and the replay does not assert every required source write; auth DB and other-character online effects are not snapshotted; group reward packet/state replay is incomplete; and compatible-client acceptance has not been run.

The inventory delta gate now records item GUIDs, owners, bag/slot links, item entry/count, item-template validity, and complete item-instance digests. It accepts only buyback deletion from bag 0 slots 74–85, one feed item whose GUID is independently checked by the feed replay, and source invalid-entry/count/template cleanup; it rejects other additions, moves, field edits, item loss, and stack changes. References with no matching `item_instance` row remain unchanged, matching TrinityCore’s `CHAR_SEL_CHARACTER_INVENTORY` inner join. Self-checks cover invalid cleanup, dangling/zero GUID links, feed stack decrement/removal, buyback deletion, and unrelated loss; all 27 isolated logout replays and a DBC/world-food feed replay pass.
